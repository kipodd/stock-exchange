class CompaniesComparer {
  ...
  addCompany(company) {
    if (!this.symbols.includes(company.symbol) && this.symbols.length <= 2) {
      ...
      this.stocksComparisionBar.insertAdjacentHTML(
        `beforeend`,
        `<button id="${company.symbol}StockButton" type="button" class="btn btn-light mr-2">${company.symbol} &times;</button>`
      );
      const stockButton = document.getElementById(
        `${company.symbol}StockButton`
      );
    ...
    if (this.symbols.length >= 2) {
      const compareCompaniesButton = document.getElementById(
        `compareCompaniesButton`
      );
